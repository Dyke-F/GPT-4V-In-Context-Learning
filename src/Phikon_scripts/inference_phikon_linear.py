...
def inference_phikon_linear(task: str, epoch: str, config: dataclass, phikon_out_dim: int = 768):
    
    model_to_load = f"TrainedPhikonLinear/PHIKON_LINEAR_{task}_{epoch}epochs_best.pth"
    ...
    results_df.to_csv(f"PHIKON_LINEAR_{task_name}_{epoch}epochs_eval.csv", index=False)
