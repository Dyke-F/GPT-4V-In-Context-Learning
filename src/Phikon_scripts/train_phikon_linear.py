...
class MHISTDataset(Dataset):
    ...
    def __init__(self, image_paths, phikon_image_processor, img_transform, exclude_filenames=None):
        """
        Args:
        ...
        self.phikon_image_processor = phikon_image_processor
    ...
    def __getitem__(self, idx):
        ...
        image = self.phikon_image_processor(image, return_tensors="pt")
        image = self.img_transform(image) # identity
...
def train_phikon_linear(task: str, config: dataclass, phikon_out_dim: int = 768):

    phikon_image_processor = AutoImageProcessor.from_pretrained("owkin/phikon")
    ...
    train_dataset = dataset_cls(
        ...
        phikon_image_processor=phikon_image_processor,
    ...
    # freeze Phikon
    for param in phikon.parameters():
        param.requires_grad_(False)

    print(
        "Phikon # trainable Parameters: ",
        sum([p.numel() for p in phikon.parameters() if p.requires_grad]),
    )
    ...
    lin = nn.Linear(phikon_out_dim, num_classes)
    print(
    ...
    phikon = phikon.to(config.backend)
    ...
    for epoch in range(config.num_epochs):
        ...
        for (
            ...
            # hacky way to remove the empty bsz, 1, c,h,w dim from the dataloader returned images
            images["pixel_values"] = images["pixel_values"].squeeze()

            with torch.inference_mode():
                outputs = phikon(**images)
                features = outputs.last_hidden_state[:, 0, :]  # (1, 768) shape

            phikon_outputs = features.detach().clone()

            # train the linear layer
            optimizer.zero_grad()
            outputs = lin(phikon_outputs)
        ...
        val_loss, val_acc = validate(phikon, lin, valid_dataloader, criterion, config)
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(lin.state_dict(), f"PHIKON_LINEAR_{task_name}_{config.num_epochs}epochs_best.pth")
            print(
...
def validate(phikon, lin, valid_dataloader, criterion, config):
    phikon = phikon.eval()
    lin = lin.eval()
    val_loss = 0.0
    correct = 0
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run Phikon linear downstream model training."
    )
    parser.add_argument(
        "task",
    ...
    print(f"Running Phikon training on task: {task}...")

    print(f"CONFIGURATION:\n", config, "\n\n")

    train_phikon_linear(task, config)
