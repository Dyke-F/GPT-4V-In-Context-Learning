...
def inference(vision_model: str, task: str, config: dataclass):
    
    model_to_load = f"TrainedClassifiers/{vision_model}_{task}_best.pth"
    ...
    model = timm.create_model(vision_model, pretrained=False, num_classes=num_classes)
    
    print(f"Loading pretrained weights from {model_to_load}")
    model.load_state_dict(torch.load(model_to_load, map_location=config.backend))
    ...
    results_df.to_csv(f"{vision_model}_{task_name}_eval.csv", index=False)


config = Conf()
models = ["resnet18", "resnet50", "tiny_vit_21m_224", "vit_small_patch8_224"]
